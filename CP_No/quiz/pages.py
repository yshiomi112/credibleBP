    ...
    def vars_for_template(self):
        ...
        for idx, text in enumerate(self.option_labels, 1):
            if idx in self.explanations:
                opts.append({'text': text, 'result': '誤', 'exp': self.explanations[idx]})
            else:
                opts.append({'text': text, 'result': '正', 'exp': ''})
