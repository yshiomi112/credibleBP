...
class LyingAversion(Page):
    form_model  = 'player'
    form_fields = ['selected_card', 'reported_number']
    ...
    def vars_for_template(self):
        # 参加者ごとに一度だけシャッフルし、変数に保存
        if 'card_order' not in self.participant.vars:
            self.participant.vars['card_order'] = random.sample(
                range(1, C.NUM_CARDS + 1), C.NUM_CARDS
            )
        return dict(numbers=self.participant.vars['card_order'])

    def error_message(self, values):
        if values.get('selected_card') is None:
            return "10 枚のカードから 1 枚選んでください。"
